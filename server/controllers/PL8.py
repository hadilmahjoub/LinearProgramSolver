...
class PL8:
    ...
    def run(self):
        ...
        # Fonction objectif : minimiser la somme des coûts des arcs utilisés
        model.setObjective(gp.quicksum(self.costs[i][j] * x[i,j] for i in range(10) for j in range(10) if not (self.costs[i][j] ==0)), gp.GRB.MINIMIZE)
        ...
        for k in range(1, 9):
            model.addConstr(gp.quicksum(x[i,k] for i in range(10) if self.costs[i][k] != 0) == gp.quicksum(x[k,j] for j in range(10) if self.costs[k][j] != 0))
