...
class PL4:
    ...
    def run(self):
        ...
        # for i in range(4):
        #     print("\n--------------------------------------\n")
        #     print("Nombre d'heures supplémentaires par mois [{}] = {}".format(i+1, int(NHS[i].x)))
        #     print("Nombre de paires de chaussures fabriquées par mois [{}] = {}".format(i+1, int(NCH[i].x)))
        #     print("Nombre d'ouvriers recrutés par mois [{}] = {}".format(i+1, int(NOR[i].x)))
        #     print("Nombre d'ouvriers licenciés par mois [{}] = {}".format(i+1, int(NOL[i].x)))
        #     print("Stock de paires de chaussures par mois [{}] = {}".format(i+1, int(S[i].x)))
        #     print("Nombre d'ouvriers disponibles par mois [{}] = {}".format(i+1, int(NO[i].x)))
        #     print("\n--------------------------------------\n")

        for i in range(4):
            result["mois"+str(i+1)]={
                "NSH" : "Nombre d'heures supplémentaires par mois [{}] = {}".format(i+1, int(NHS[i].x)),
                "NCH" : "Nombre de paires de chaussures fabriquées par mois [{}] = {}".format(i+1, int(NCH[i].x)),
                "NOR" : "Nombre d'ouvriers recrutés par mois [{}] = {}".format(i+1, int(NOR[i].x)),
                "NOL" : "Nombre d'ouvriers licenciés par mois [{}] = {}".format(i+1, int(NOL[i].x)),
                "S" : "Stock de paires de chaussures par mois [{}] = {}".format(i+1, int(S[i].x)),
                "NO" : "Nombre d'ouvriers disponibles par mois [{}] = {}".format(i+1, int(NO[i].x))
            }
        return result
